#!/usr/bin/env python3
"""아카이브(raw/all_html.json)에서 뱅크나무 글을 뽑아 content/posts.json 으로 저장한다.
...
CAT_NAME = {"deposit": "예적금", "card": "카드", "loan": "대출", "insurance": "보험",
            "invest": "투자", "tax": "세금", "tip": "생활정보", "bank": "금융"}
# 옛 사이트 카테고리 목록 페이지 -> 카테고리. 하위 카테고리(예적금 등) 페이지가 상위(금융)보다 우선한다.
LISTING_CAT = {"/category/bank/deposit/": "deposit", "/category/bank/deposit/page/2/": "deposit",
               "/category/bank/card/": "card", "/category/bank/loan/": "loan",
               "/category/bank/insurance/": "insurance", "/category/invest/": "invest",
               "/category/tax/": "tax", "/category/tip/": "tip",
               "/category/bank/": "bank", "/category/bank/page/2/": "bank"}
MANUAL_CAT = {}
IMG_DIR = "images"
...
def build_category_map():
    ...
    # 글 본문의 <article class="... category-xxx"> 가 가장 정확하다
    for url, doc in RAW.items():
        m = re.search(r'<article[^>]*class="[^"]*\bcategory-([a-z-]+)', doc)
        if m and m.group(1) in CAT_NAME:
            slug = urllib.parse.unquote(urllib.parse.urlparse(url).path).strip("/")
            cat[slug] = m.group(1)
    return cat
...
def clean_body(body: str) -> str:
    ...
    # Shortcodes Ultimate 상자 -> 단순 강조 상자
    body = re.sub(r'<div class="su-box-title"[^>]*>(.*?)</div>', r'<p class="box-title">\1</p>', body, flags=re.S)
    body = re.sub(r'<div class="su-box-content[^"]*"[^>]*>', '<div class="box-content">', body)
    body = re.sub(r'<div[^>]*class="su-box[^"]*"[^>]*>', '<div class="box">', body)
    body = re.sub(r'<div[^>]*class="wp-block-spacer"[^>]*></div>', '', body)
    ...
    body = ATTR_DROP.sub(lambda m: m.group() if re.search(r'"(?:wp-block-table|box|box-title|box-content)"', m.group()) else "", body)
...
def extract(url: str, doc: str, ts: str):
    ...
    cat_slug = CATEGORY_OF.get(slug, "tip")
    ...
    body = re.sub(r'href="https?://(?:www\.)?banknamu\.com/([^"]*)"',
                  lambda m: 'href="/' + urllib.parse.unquote(m.group(1)) + '"', body)
    text = re.sub(r"\s+", " ", re.sub(r"<[^>]+>", " ", body)).strip()
    return {
        "slug": slug, "original_url": f"https://banknamu.com{path}", "archive_ts": ts,
...
for ts, url, *_ in ROWS:
    doc = RAW.get(url, "")
    # 실제 글만: <article class="post-NNN post type-post ..."> (목록·고정 페이지 제외)
    if not re.search(r'<body[^>]*class="[^"]*\bpostid-\d+', doc):
        continue
