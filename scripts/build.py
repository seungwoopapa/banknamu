...
ALL_CATS = [("deposit", "예적금"), ("card", "카드"), ("loan", "대출"), ("insurance", "보험"),
            ("invest", "투자"), ("tax", "세금"), ("tip", "생활정보"), ("bank", "금융")]
# 글이 하나도 없는 카테고리는 메뉴·페이지를 만들지 않는다
CATS = [(s, n) for s, n in ALL_CATS if any(p["category_slug"] == s for p in POSTS)]
CAT_DESC = {
    "deposit": "파킹통장·적금 금리 비교, 청년도약계좌, 청약통장, 저축은행 등 예금·적금 상품과 계좌 관련 글입니다.",
    "card": "신용카드·체크카드 혜택과 사용 요령을 정리한 글입니다.",
    "loan": "대출 상품과 조건, 이용 시 주의할 점을 정리한 글입니다.",
    "insurance": "자동차보험 비교, 보험료 계산, 할인·할증 등급 등 보험 관련 글입니다.",
    "invest": "투자 상품과 방법을 정리한 글입니다.",
    "tax": "이자소득세 등 금융과 관련된 세금 제도를 정리한 글입니다.",
    "tip": "청년 지원금, 사업자 통장, 지급정지 계좌 등 생활에 도움이 되는 금융 정보입니다.",
    "bank": "은행 이용과 관련된 금융 상식입니다.",
}
...
<header class="site-header">
  <div class="wrap">
    <a class="brand" href="/"><span class="brand-mark">B</span> {esc(CONF['site_name'])}</a>
...
<footer class="site-footer">
  <div class="wrap">
    ...
    <p>© {CONF.get("start_year", "2023")}–{YEAR} {esc(CONF['site_name'])}. 이 사이트의 글은 작성자의 개인적인 공부와 경험을 정리한 것으로, 특정 금융상품의 매수·매도를 권유하지 않습니다.</p>
  </div>
</footer>
</body>
</html>
...
def build_post(i, p):
    ...
    notice = (f'<aside class="notice"><strong>작성일 {kdate(p["published"])}</strong>'
              ...
              + f'<br>이 글의 금리·상품 조건·제도·수치는 <b>{p["published"][:4]}년 {int(p["published"][5:7])}월 작성 시점 기준</b>입니다. 현재 기준과 다를 수 있으니 실제 결정 전에는 해당 기관의 최신 안내를 확인해 주세요.</aside>')
...
def build_pages():
    for page in ["about", "contact", "privacy", "disclaimer"]:
        ...
        raw = raw.replace("{{email}}", CONF["contact_email"] or "(이메일 준비 중)").replace("{{site_name}}", CONF["site_name"]) \
